...
//   const {setSelectedUserUID} = useChat();
//   // Use Context to set selected user
...
//     setSelectedUserUID(user.LoggedUserId)
...
import { onAuthStateChanged } from "firebase/auth";
import { useChat } from "../../context/ChatContext";
import { collection, query, onSnapshot, where, getDocs, setDoc, doc, getDoc, serverTimestamp } from "firebase/firestore";
...
const AllChat = () => {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      const unsubscribeFirestore = onSnapshot(q, (querySnapshot) => {
        ...
        // Filter out the currently logged-in user
        const filteredUsers = allUsers.filter((user) => user.LoggedUserId !== currentUser.uid);
  ...
  const handleClick = async (user) => {
    ...
    const chatRoomId = generateChatRoom(currentUser.uid, user.LoggedUserId);
    const chatRef = doc(db, "chatRooms", chatRoomId);
    const chatDoc = await getDoc(chatRef);
  ...
  return (
    <div className="h-[500px] overflow-y-auto p-2">
      {users.length > 0 ? (
        users.map((user) => (
          <div key={user.LoggedUserId} className="flex gap-4 mt-3 p-2 border-b cursor-pointer" onClick={() => handleClick(user)}>
            <p className="font-semibold">{user.name}</p>
          </div>
        ))
      ) : (
